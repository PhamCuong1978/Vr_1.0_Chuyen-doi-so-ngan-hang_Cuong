...
export default function App() {
    ...
    const handleExtractText = async () => {
        ...
        // Kiểm tra xem có file ảnh/PDF không
        const hasImagesOrPDF = selectedFiles.some(f => f.type.startsWith('image/') || f.type === 'application/pdf');
        
        setLoadingState('extracting');
        setError(null);
        
        // Cập nhật thông báo trạng thái dựa trên loại file
        if (hasImagesOrPDF) {
            startProgress("Gemini Flash đang đọc ảnh (OCR)...");
        } else {
            startProgress("Đang đọc dữ liệu văn bản từ file (Local)...");
        }
        ...
        try {
            ...
            // Chỉ gọi AI OCR nếu thực sự có ảnh
            ...
            finishProgress();
            // Nếu là file text thuần, force state về idle nhanh hơn
            if (!hasImagesOrPDF) {
                setLoadingState('idle');
                setProgress(0);
            }
    ...
    const startProgress = (message: string) => {
        setProgress(0);
        if (progressInterval.current) clearInterval(progressInterval.current);
        // Lưu message vào state hoặc biến tạm nếu cần hiển thị dynamic (hiện tại dùng getLoadingMessage)
    ...
    const getLoadingMessage = () => {
        // Kiểm tra loại file để hiển thị message phù hợp
        const hasImagesOrPDF = selectedFiles.some(f => f.type.startsWith('image/') || f.type === 'application/pdf');

        switch(loadingState) {
            case 'extracting': 
                return hasImagesOrPDF 
                    ? `Gemini Flash đang đọc ảnh... ${Math.round(progress)}%` 
                    : `Đang đọc dữ liệu văn bản từ file...`;
            ...
            <div className="max-w-7xl mx-auto">
                <header className="text-center mb-8">
                    ...
                    <p className="mt-2 text-gray-600 dark:text-gray-400 flex items-center justify-center gap-2">
                        <span>Upload sao kê (Excel, PDF, Ảnh), kiểm tra số dư và nhận ngay bảng dữ liệu.</span>
                            ...
                            <div className="mb-6">
                                ...
                                >
                                    ...
                                    {loadingState === 'extracting' 
                                        ? 'Đang đọc dữ liệu...' 
                                        : `Trích xuất dữ liệu (${selectedFiles.length} file)`
                                    }
                                </button>
                                {selectedFiles.length > 0 && !selectedFiles.some(f => f.type.startsWith('image/') || f.type === 'application/pdf') && (
                                    <p className="mt-2 text-xs text-green-600 italic text-center">
                                        <span className="font-bold">Mẹo:</span> File Excel/Word sẽ được đọc trực tiếp (Nhanh & Không tốn phí AI).
                                    </p>
                                )}
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
                        ...
                        <ul className="space-y-4 text-gray-600 dark:text-gray-400">
                            <li className="flex items-start">
                                ...
                                <span><b>Upload File:</b> Hỗ trợ đa dạng các loại file: Excel, Word, PDF, Ảnh.</span>
                            </li>
                            <li className="flex items-start">
                                ...
                                <span><b>Trích xuất:</b> Với Excel/Word, hệ thống đọc ngay lập tức (Local). Với Ảnh/PDF, AI sẽ hỗ trợ OCR.</span>
                            </li>
                             <li className="flex items-start">
                                ...
                                <span><b>Kiểm tra:</b> Kiểm tra nội dung văn bản trước khi xử lý nghiệp vụ.</span>
                            </li>
                            <li className="flex items-start">
                                ...
                                <span><b>Xử lý Nghiệp vụ:</b> DeepSeek AI sẽ phân tích và tạo bảng kế toán chi tiết.</span>
                            </li>
                        </ul>
                    </div>
                </div>
